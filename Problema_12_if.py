'''“Mă iubeşte un pic, mult, cu pasiune, la nebunie, de loc, un pic,…”. Rupând petalele unei margarete cu x petale, el (ea) mă
iubeşte …. Exemplu: Date de intrare: x=10 Date de ieşire: … de loc.'''
x=int(input('Numarul de petale:'))
if (x%5)==1:
    print("El/ea ma iubeste un pic")
if (x%5)==2:
    print("El/ea ma iubeste mult")
if (x%5)==3:
    print("El/ea ma iubeste cu pasiune")
if (x%5)==4:
    print("El/ea ma iubeste la nebunie")
else:
    print("El/ea ma iubeste deloc")
